...
async def remove_note(note_id: int, db: Session) -> Union[Note, None]:
...
async def update_note(note_id: int, body: NoteUpdate, db: Session) -> Union[Note, None]:
...
async def update_status_note(note_id: int, body: NoteStatusUpdate, db: Session) -> Union[Note, None]:
    note = db.query(Note).filter(Note.id == note_id).first()
    if note:
        note.done = body.done
        db.commit()
